...
const getUrl=(id)=>{return links.find((link)=>link.id===id).url}
    ...
    // const news=testData
        if(findBy){
            if(Array.isArray(findBy)){
                for (let newsItem of news){
                    for(let item of newsItem.items){
                        ...
                        for(let filter of findBy){
                            item.title.toLowerCase().split(' ').includes(filter.toLowerCase()) && rating++                              
                        }
                        if(rating !==0){
                            articles.push({
                                rating:rating,
                ...
                for (let newsItem of news){
                    for(let item of newsItem.items){
                        item.title.toLowerCase().split(' ').includes(findBy.toLowerCase()) &&     
                            articles.push({
                                rating:1,
                                title:item.title,
                                pubDate:item.pubDate,
